...
class EmulationProcessor {
    ...
    async setGeolocationOverride(params) {
        if ((params.coordinates?.altitude ?? null) !== null) {
            throw new ErrorResponse_1.UnsupportedOperationException('Geolocation altitude emulation is not supported');
        }
        if ((params.coordinates?.heading ?? null) !== null) {
            throw new ErrorResponse_1.UnsupportedOperationException('Geolocation heading emulation is not supported');
        }
        if ((params.coordinates?.altitudeAccuracy ?? null) !== null) {
            throw new ErrorResponse_1.UnsupportedOperationException('Geolocation altitudeAccuracy emulation is not supported');
        }
        if ((params.coordinates?.speed ?? null) !== null) {
            throw new ErrorResponse_1.UnsupportedOperationException('Geolocation speed emulation is not supported');
        }
    ...
    async #getRelatedTopLevelBrowsingContexts(browsingContextIds, userContextIds) {
        if (browsingContextIds === undefined && userContextIds === undefined) {
            throw new ErrorResponse_1.InvalidArgumentException('Either user contexts or browsing contexts must be provided');
        }
        if (browsingContextIds !== undefined && userContextIds !== undefined) {
            throw new ErrorResponse_1.InvalidArgumentException('User contexts and browsing contexts are mutually exclusive');
        }
        const result = [];
        if (browsingContextIds === undefined) {
            // userContextIds !== undefined
            if (userContextIds.length === 0) {
                throw new ErrorResponse_1.InvalidArgumentException('user context should be provided');
            }
            ...
            if (browsingContextIds.length === 0) {
                throw new ErrorResponse_1.InvalidArgumentException('browsing context should be provided');
            }
            for (const browsingContextId of browsingContextIds) {
                const browsingContext = this.#browsingContextStorage.getContext(browsingContextId);
                if (!browsingContext.isTopLevelContext()) {
                    throw new ErrorResponse_1.InvalidArgumentException('The command is only supported on the top-level context');
                }
